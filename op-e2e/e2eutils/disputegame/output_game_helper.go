...
func (g *OutputGameHelper) ClaimCredit(ctx context.Context, addr common.Address) {
	...
	tx, err := g.game.ClaimCredit(g.opts, addr)
	g.require.NoErrorf(err, "Failed to claim credit for %v", addr)
	_, err = wait.ForReceiptOK(ctx, g.client, tx.Hash())
	g.require.NoError(err)
}

func (g *OutputGameHelper) WethUnlockPeriod(ctx context.Context) *big.Int {
