...
func TestOutputCannon_ReclaimBond(t *testing.T) {
	...
	// Advance the time past the weth delay with an extra 2 minute buffer
	wethDelay := game.WethUnlockPeriod(ctx)
	delay, _ := wethDelay.Float64()
	duration := time.Duration(delay)*time.Second + 120*time.Second
	sys.TimeTravelClock.AdvanceTime(duration)
	require.NoError(t, wait.ForNextBlock(ctx, l1Client))

	// Wait for alice to have no available credit
	// aka, wait for the challenger to claim its credit
	// TODO: this wait isn't working... for some reason the challenger isn't claiming credits
	// game.WaitForNoAvailableCredit(ctx, sys.Cfg.Secrets.Addresses().Alice)

	// Try to manually claim the credit for alice
	t.Logf("Attempting to claim credit for alice after duration: %v", duration)
	game.ClaimCredit(ctx, sys.Cfg.Secrets.Addresses().Alice)
